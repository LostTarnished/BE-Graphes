...
public class Path {
        ...
        for (int i = 0; i < nodes.size() - 1; i++) {
            ...
            for (Arc arc : current.getSuccessors()) {
                if (arc.getDestination() == next) {
                    if (bestArc == null || arc.getMinimumTravelTime() < bestArc
    ...
    public boolean isValid() {
        if (size() > 2) {
            for (int i = 0; i < this.arcs.size(); i++) {
                ...
                if (i == 0) {
                    if (arc.getOrigin() != this.origin) {
                        return false;
                    }
                }
                else {
                    if (arc.getOrigin() != this.arcs.get(i - 1).getDestination()) {
                        return false;
                    }
                }
            }
    ...
    public double getTravelTime(double speed) {
        return 3.6 * this.getLength() / speed;
    }
